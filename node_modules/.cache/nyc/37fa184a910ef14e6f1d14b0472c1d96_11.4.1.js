...
=
	...
	reorderField(incidentId, fieldId, order)
	{
		...
		const promise = this.loadOne(fieldId).then(field => {
			...
			const promise = IncidentField.update(findQuery, updateQuery, {multi: true}).then(() => {
	...
	{
		const BuildingAgencyIncidentDao = require('./../../building-agency/models/building-agency-incident-dao');
		const buildingAgencyIncidentDao = new BuildingAgencyIncidentDao();
		buildingAgencyIncidentDao.updateAll();
	}
}
